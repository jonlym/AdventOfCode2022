function main()
    ...
    final_position = 0;
    for end_position in message_start_marker_length:length(message)
        start_position = end_position - message_start_marker_length + 1;
        unique_characters = Set(message[start_position:end_position]);
        if length(unique_characters) == message_start_marker_length
