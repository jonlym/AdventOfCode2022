function main()
    ...
    while !eof(f_ptr)
        ...
        overlap = check_if_overlap(assignments1, assignments2);
        if overlap
            n_subsets += 1;
        end
    end
...
function check_if_overlap(assignments1, assignments2)
    overlap = false;
    if (assignments1[1] <= assignments2[1]) && (assignments2[1] <= assignments1[2]) 
        overlap = true;
    elseif (assignments2[1] <= assignments1[1]) && (assignments1[1] <= assignments2[2])
        overlap = true;
    end
    return overlap;
end
